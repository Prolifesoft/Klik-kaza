...
export function Ads() {
  ...
  const claimReward = async () => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({ type: 'success', text: `+${data.credits_earned} Kredi kazandınız!` });
        // Update user state
  ...
  if (loading) return <div className="p-8 text-center text-zinc-500">Reklamlar yükleniyor...</div>;

  return (
    <div className="space-y-6">
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-zinc-900 tracking-tight">Reklamlar</h1>
        <p className="text-zinc-500 mt-1">İzle ve kazanmaya başla</p>
      </header>
